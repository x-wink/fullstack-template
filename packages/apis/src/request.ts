...
const showError =
  import.meta.env.VITE_BASE_URL.indexOf('admin') === -1 ? showToast : ElMessage.error
...
export const request = async <ResData, ReqBody = unknown>(
  ...
  const res = await instance.request<Res<ResData>, AxiosResponse<ResData> | undefined, ReqBody>(
    config,
  )
  return res?.data
}
...
export const post = <ResData, ReqBody = unknown>(
  url: string,
  data?: ReqBody,
  config?: AxiosRequestConfig<ReqBody>,
) =>
...
export const put = <ResData, ReqBody = unknown>(
  url: string,
  data?: ReqBody,
  config?: AxiosRequestConfig<ReqBody>,
) =>
